...
| Repeat Status.                | Indicates whether a transaction is a repeat sale for the customer. This KPI uses standardized logic based on the repeat_status field and includes all slabs. The logic is now aligned across both export and report outputs. |
