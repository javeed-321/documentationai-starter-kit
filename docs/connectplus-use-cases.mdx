...
Connect+ converts each CSV row into a JSON object and passes the full array to the jslt_transform block before sending the data to the destination.
...
| --- | --- |
| `for (.)` | Iterates over every row in the input array and produces one output object per row **Note**: Place the `for` loop at the beginning. |
...
| `.loyaltyType` | Maps to `loyaltyType` inside the `loyaltyInfo` object |
...
| Jane      | Doe      | [jane.doe@example.com](mailto:jane.doe@example.com) | INSTORE | email          |
