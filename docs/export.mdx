...
When you include custom fields (such as user_cf_is_verified) as dimensions or measures in your export template, only users who have records for these fields are included in the export. Users without these custom field records are excluded, which can result in fewer rows than expected. To export the full user set, avoid including such custom fields as dimensions or measures.
