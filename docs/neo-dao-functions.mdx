...
The `getFunctionArguments()` function retrieves the list of arguments passed to a [helper function block ](https://docs.capillarytech.com/docs/block-libraries#/reusing-logic-in-a-script-block)during execution using `executeBlock()`. It returns an array of arguments in the order they were passed.
...
<Callout kind="info" title="dagType">
