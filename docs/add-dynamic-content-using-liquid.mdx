...
While creating a template in the **Creatives** section for **SMS**, **Push Notification**, **In-App**, and **Email** channels, Liquid expressions are not strictly validated at the time of creation. You can add any Liquid expressions, save the template without errors, and use **Test and Preview** within Creatives. However, ensure that all curly braces are properly closed; improperly closed `{{` or `{%` tags will still throw a validation error when saving the template.
